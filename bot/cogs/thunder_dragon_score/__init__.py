...
class ThunderDragonScore(commands.Cog):
    ...
    async def rob_prefix(self, ctx: commands.Context, member: discord.Member):
        
        if (cd := Core.command_cooldown.get(self.rob_prefix.name, {}).get(ctx.author.id, 0) - datetime.now().timestamp()) > 0:
        ...
        if (cd := Core.remaining_time.get(ctx.author.id, 0) - datetime.now().timestamp()) > 0:
            await ctx.reply(f"十分不幸的，同志，你被遣送古拉格（或是被雷普了）！請等待 {round(cd)} 秒後再試。", mention_author=False)
            return None
        ...
        Core.command_cooldown[self.rob_prefix.name][ctx.author.id] = datetime.now().timestamp() + ROB_COOLDOWN
        
        log.debug(f"{ctx.author.name} used rob command. target: {member.name}")
        
    
    @commands.command(name="rape")
    async def rape_prefix(self, ctx: commands.Context, member: discord.Member):
        
        if (cd := Core.command_cooldown.get(self.rape_prefix.name, {}).get(ctx.author.id, 0) - datetime.now().timestamp()) > 0:
        ...
        Core.command_cooldown[self.rape_prefix.name][ctx.author.id] = datetime.now().timestamp() + RAPE_COOLDOWN
    ...
    async def work_prefix(self, ctx: commands.Context):
        ...
        Core.command_cooldown[self.work_prefix.name][ctx.author.id] = datetime.now().timestamp() + WORK_COOLDOWN
        
        log.debug(f"{ctx.author.name} used work command.")
    
    
    @commands.command(name="useringo")
    async def use_ringo_prefix(self, ctx: commands.Context, amount: int=1):
